...
def channels_inline_kb(missing: list) -> InlineKeyboardMarkup:
    """Inline buttons — one per missing channel (join links only, no check button)."""
    rows = []
    for ch in missing:
        link    = ch.get("link", "")
...
def joined_reply_kb() -> ReplyKeyboardMarkup:
    """Reply keyboard shown while user needs to join channels."""
...
async def send_welcome(bot, chat_id: int, first_name: str) -> None:
    txt = (
        ...
        "║   🔓  HTML DECODER BOT   ║\n"
        "╚━━━━━━━━━━━━━━━━━━━━━━━╝\n\n"
        f"👋 Hey <b>{first_name}</b>, welcome aboard!\n\n"
        "I can instantly decode your encrypted HTML files.\n\n"
        "🔥 <b>Supported Engines</b>\n"
        "┣ ⚡ <b>phpkobo</b>  — auto-detected\n"
        "┗ 🌐 <b>Browser</b>  — standard encryption\n\n"
        "━━━━━━━━━━━━━━━━━━━━━━━\n"
...
def ban_text(uid: int) -> str:
    return (
        ...
        "If you believe this is a mistake, contact the Owner with your ID to request an unban."
    )
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not joined:
        ...
        await update.message.reply_html(
            ...
            f"You need to join <b>{n}</b> channel{'s' if n > 1 else ''} to use this bot.\n\n"
            f"👇 Join the channels below, then press the button:",
            reply_markup=joined_reply_kb(),
        )
        await update.message.reply_text("📢 Channels:", reply_markup=channels_inline_kb(missing))
        return
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── JOINED check (reply keyboard button) ──────────────────────────
    if "I've Joined All Channels" in text:
        # Acknowledge private channels
        ...
        if not joined:
            n = len(missing)
            await update.message.reply_html(
                f"❌ You haven't joined all channels yet!\n\nStill <b>{n}</b> channel{'s' if n > 1 else ''} remaining:",
                reply_markup=joined_reply_kb(),
            )
            await update.message.reply_text("📢 Channels:", reply_markup=channels_inline_kb(missing))
            return
    ...
    # ── Menu buttons ────────────────────────────────────────────────────
    if "Decode HTML" in text:
        joined, missing = await check_channels(context.bot, user.id)
        if not joined:
            n = len(missing)
            await update.message.reply_html(
                f"📋 <b>Channel Verification</b>\n\nJoin <b>{n}</b> channel{'s' if n > 1 else ''} first:",
                reply_markup=joined_reply_kb(),
            )
            await update.message.reply_text("📢 Channels:", reply_markup=channels_inline_kb(missing))
            return
    ...
    # Unknown text → silently ignore (bot only responds to menu buttons)
...
# ════════════════════════════════════════
#        PHPKOBO DECRYPT ENGINE
#   Ported from Phpkobo_New_Script_v2.js
# ════════════════════════════════════════
def _js_unescape(s: str) -> str:
    """Emulate JS:  eval('"' + s.replace(/\\n/g, '\\\\n') + '"')"""
    s = s.replace('\n', '\\n')
...
def detect_phpkobo(html: str) -> bool:
    """Return True if the file looks like phpkobo-encrypted."""
...
def decode_phpkobo(html: str) -> str:
    """
    Decode a phpkobo-encrypted HTML string.
    Algorithm ported 1-to-1 from Phpkobo_New_Script_v2.js
    """
    # Step 1 — Extract <script> content
    sm = re.search(r'<script[^>]*>([\s\S]*?)<\/script>', html, re.IGNORECASE)
    if not sm:
        raise ValueError("No <script> tag found")
    sc = sm.group(1).lstrip(';').strip()

    # Step 2 — Locate Function("…")() or Function('…')() wrapper
    if 'Function("' in sc:
        s = sc.index('Function("') + 10
        e = sc.rindex('")()')
        inner = sc[s:e]
    elif "Function('" in sc:
        s = sc.index("Function('") + 10
        ...
        raise ValueError("phpkobo Function wrapper not found")

    # Step 3 — Unescape inner string (emulate JS eval)
    code = _js_unescape(inner)

    # Step 4 — Extract _Srkpv payload string
    srkpv_pos = code.find('_Srkpv=')
    if srkpv_pos < 0:
        raise ValueError("_Srkpv payload not found")
    sp_qs = code.index('"', srkpv_pos + 7) + 1
    sp_qe = code.index('";', sp_qs)
    srkpv = code[sp_qs:sp_qe].rstrip('\\').rstrip('"')

    # Step 5 — Strip everything before (and including) the 'B' marker
    bi = srkpv.find('B')
    if bi < 0:
        raise ValueError("B marker not found in payload")
    sp = srkpv[bi + 1:].rstrip('\\').rstrip('"')

    # Step 6 — Read 8-char header from the end: [4:6]=step, [6:8]=start
    if len(sp) < 8:
        raise ValueError("Payload too short to contain header")
    hdr   = sp[-8:]
    step  = int(hdr[4:6], 16)
    start = int(hdr[6:8], 16)
    sp    = sp[:-8]

    # Step 7 — Normalise substituted hex chars (X→e, Y→a, Z→c, rest→0)
    sp = re.sub(r'[Xx]', 'e', sp)
    sp = re.sub(r'[Yy]', 'a', sp)
    sp = re.sub(r'[Zz]', 'c', sp)
    sp = re.sub(r'[^0-9a-fA-F]', '0', sp)

    # Step 8 — Split into hex byte pairs
    pairs = re.findall(r'.{2}', sp)
    if not pairs:
        raise ValueError("No hex pairs found in payload")

    # Step 9 — Build W[] lookup table  (same loop as JS)
    W = [0] * 256
    j = start
    for i in range(256):
        W[j] = i
    ...
    # Step 10 — Decrypt each byte
    byte_vals = [(int(px, 16) - W[idx % 256] + 256) % 256
                 for idx, px in enumerate(pairs)]

    # Step 11 — Decode bytes → UTF-8
    result = bytes(byte_vals).decode('utf-8', errors='replace')
    if len(result) < 30:
        raise ValueError("Decoded result is too short — decryption may have failed")
    return result
...
async def _do_decode(update: Update, context: ContextTypes.DEFAULT_TYPE, doc, fname: str) -> None:
    ...
    for s in ["⬇️  Downloading file...", "🔒 Running security check...", "📋 Inspecting file..."]:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # ── Read file & smart-detect encryption type ──────────────────
            with open(in_path, "r", encoding="utf-8", errors="replace") as f:
                raw_html = f.read()

            is_phpkobo = detect_phpkobo(raw_html)

            # ══════════════════════════════════════════
            #   PATH A — PHPKOBO ENCRYPTED
            # ══════════════════════════════════════════
            if is_phpkobo:
                phpkobo_steps = [
                    "🔍 ▱▱▱▱▱▱▱  Detected: phpkobo encryption",
                    "🔑 ▰▱▱▱▱▱▱  Locating Function wrapper...",
                    "📦 ▰▰▱▱▱▱▱  Extracting payload...",
                    "🧩 ▰▰▰▱▱▱▱  Finding B-marker...",
                    "📐 ▰▰▰▰▱▱▱  Reading cipher header...",
                    "🔓 ▰▰▰▰▰▱▱  Decrypting bytes...",
                    "🛠️ ▰▰▰▰▰▰▱  Building output...",
                    "💾 ▰▰▰▰▰▰▰  Saving file...",
                ]
                for step_txt in phpkobo_steps:
                    try: await msg.edit_text(step_txt)
                    except Exception: pass
                    await asyncio.sleep(0.6)

                decoded_html = decode_phpkobo(raw_html)
                method_label = "⚡ Decoded via phpkobo engine"

            # ══════════════════════════════════════════
            #   PATH B — NORMAL / UNKNOWN ENCRYPTION
            #            → Browser rendering
            # ══════════════════════════════════════════
            else:
                browser_steps = [
                    "🌐 ▱▱▱▱▱▱▱  Encryption type: standard",
                    "🖥️ ▰▱▱▱▱▱▱  Starting browser server...",
                    "🖥️ ▰▰▱▱▱▱▱  Loading page...",
                    "🖥️ ▰▰▰▱▱▱▱  Browser running...",
                    "🔍 ▰▰▰▰▱▱▱  Scanning HTML...",
                    "✨ ▰▰▰▰▰▱▱  Rendering & decoding...",
                ...
                i    = 0
                while not task.done():
                    try: await msg.edit_text(browser_steps[i % len(browser_steps)])
                    except Exception: pass
                    i += 1
                    await asyncio.sleep(1.5)

                decoded_html = await task
                method_label = "🌐 Decoded via browser rendering"
        ...
        try: await msg.edit_text(f"❌ Decode failed!\n\n<code>{e}</code>", parse_mode=ParseMode.HTML)
        except Exception: pass
